...
class VLAN(Conf):
    ...
    def reset_ports(self, ports):
        ...
        self.untagged = tuple(
            port
            for port in sorted_ports
            if (self.is_same_vlan(port.native_vlan) and port.dyn_dot1x_native_vlan is None)
        )
        self.dot1x_untagged = tuple(
            port for port in sorted_ports if self.is_same_vlan(port.dyn_dot1x_native_vlan)
        )
