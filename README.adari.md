...
| `upstream/<topic>` | one per change, off `main`, holding only that change. What a PR to `faucetsdn/faucet` is opened from. |

## Carried patches

Each is independently applicable — they touch disjoint files — so each gets
its own upstream PR.

| Topic branch | Changes | Upstream issue tracked in adarigod |
|---|---|---|
| `upstream/reset-ports-by-vid` | `VLAN.reset_ports` compares VIDs instead of hashing whole VLAN configs | adarigod#2735 |
| `upstream/reset-refs-by-vid` | `DP.reset_refs` groups ports by VID and port number | adarigod#2736 |
| `upstream/arp-padding` | `PacketMeta.packet_complete` accepts a fixed-size protocol whose padding was discarded | adarigod#2737 |
| `upstream/routed-vlans-by-vid` | `ValveRouteManager` looks a VLAN's routers up by VID | adarigod#2760 |
...
own suite goes `test_vlan` 12 → 16, `test_config` 204 → 206,
`test_valve_packet` 0 → 4, `test_valve_route` 0 → 5, all passing.
